...
pub(crate) fn call_python_handler(
    ...
    let has_query_args = param_names
        .iter()
        .any(|n| n != "body" && n != "request" && !path_params.contains_key(n));
    ...
    let execution_res = (|| -> PyResult<PyObject> {
        ...
        if let Some(req) = py_request_wrapped_opt.as_ref() {
            let modified = crate::middleware::execute_before_middlewares(
                py,
                &state.middlewares,
                req.clone_ref(py),
            )?;
        ...
        for name in param_names {
            let is_injected = call_kwargs_opt
                .as_ref()
                .is_some_and(|k| k.contains(name).unwrap_or(false));
            if is_injected {
                continue;
            }
        ...
        // Body Injection
        let needs_body = param_names.iter().any(|n| n == "body")
            && call_kwargs_opt
                .as_ref()
                .is_none_or(|k| !k.contains("body").unwrap_or(false));
        if needs_body {
            let is_json = headers
                .iter()
                        ...
                        if let Some(ref mc) = handler_sig.pydantic_body_model {
                            ...
                            match mc.bind(py).call_method1("model_validate", (&py_obj,)) {
                                ...
                                Err(ve) => {
                                    ...
                                    return Ok(pyo3::types::PyTuple::new(
                                        py,
                                        vec![
                                            eb_obj.into_any().unbind(),
                                            sc_obj.into_any().unbind(),
                                        ],
                                    )?
                                    .into_any()
                                    .unbind());
            ...
            if param_names.contains(k) {
                ...
                if !kw.contains(k)? {
                    if let Some(ann) = handler_sig.param_types.get(k) {
                        let c = ann.bind(py).call1((v,)).unwrap_or_else(|_| {
                            v.into_pyobject(py)
                                .unwrap()
                                .into_any()
                                .unbind()
        ...
        // Call
        let res = if let Some(kw) = call_kwargs_opt {
            handler.call(py, (), Some(&kw))?
        } else {
            handler.call0(py)?
        };
            ...
            crate::server::ASYNCIO_LOOP.with(|loop_cell| {
                ...
                if loop_opt.is_none() {
                    ...
                    if let Ok(nl) = nf.bind(py).call0() {
                        ...
                        if let Ok(rm) = nl.getattr("run_until_complete") {
                            *loop_opt = Some((nl.unbind(), rm.unbind()));
                        }
                    }
                }
                if let Some(ref c) = *loop_opt {
                    ...
                    py.import("asyncio")
                        .and_then(|a| a.call_method1("run", (&res,)))
                        .map(|v| v.unbind())
                }
            })
        } else {
    ...
    if let Some(ref r) = py_request_wrapped_opt {
        final_res =
            crate::middleware::execute_after_middlewares(py, &state.middlewares, r, final_res)?;
    }
    ...
    if actual.hasattr("content_type")?
        && actual.hasattr("render")?
        && !actual.is_instance_of::<PyDict>()
        && !actual.is_instance_of::<PyString>()
    {
        ...
        let resp_headers: Option<HashMap<String, String>> =
            if let Ok(hdict) = actual.getattr("headers") {
    ...
    let (bs, ct) = if actual.is_instance_of::<PyDict>()
        || actual.is_instance_of::<pyo3::types::PyList>()
        || actual.is_instance_of::<pyo3::types::PyBool>()
